...
class ElasticsearchUploaderWithDelay:
    ...
    def __init__(self, excel_file_path: str = None, es_host: str = None, index_name: str = None, delay_seconds: int = None):
        ...
        try:
            ...
            logger.info(f"⏱️  Upload delay: {self.delay_seconds} seconds between documents")
    ...
    def _upload_documents_with_delay(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            total_docs = len(df)
            logger.info(f"📤 Starting DELAYED upload of {total_docs} documents to index: {self.index_name}")
            logger.info(f"⏱️  Delay between uploads: {self.delay_seconds} seconds")
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    logger.info(f"✅ [{success_count}/{total_docs}] Uploaded: {title}...")
                    ...
                    failed_count += 1
                    logger.error(f"❌ Failed to upload document {idx + 1}: {e}")
                    # Continue with next document even if one fails
                    continue
            
            logger.info(f"✅ Successfully uploaded {success_count} documents")
            if failed_count > 0:
                logger.warning(f"⚠️ Failed to upload {failed_count} documents")
    ...
    def _print_summary(self, original_df: pd.DataFrame, new_docs_uploaded: int) -> None:
        ...
        print("\n" + "=" * 70)
        print(f"✅ Elasticsearch upload completed (WITH DELAY)!")
        ...
        print(f"   • Upload delay used: {self.delay_seconds} seconds between documents")
        print("=" * 70)
    ...
    def upload_to_elasticsearch(self) -> None:
        ...
        try:
            ...
            if uploaded_count > 0:
                logger.info(f"🎉 Successfully uploaded {uploaded_count} new documents with {self.delay_seconds}s delay!")
            else:
...
def main():
    ...
    try:
        ...
        logger.info(f"   • Upload delay: {ElasticsearchUploaderWithDelay.UPLOAD_DELAY_SECONDS} seconds")
