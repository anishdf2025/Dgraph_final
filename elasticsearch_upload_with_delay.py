...
between each document. Useful when processing multiple documents in the same Excel file
to simulate incremental updates or avoid overwhelming the system.

Key Difference from elasticsearch_upload.py:
- Adds 30-second delay between each document upload
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('elasticsearch_upload_with_delay.log'),
...
class ElasticsearchUploaderWithDelay:
    ...
    def __init__(self, excel_file_path: str = None, es_host: str = None, index_name: str = None, delay_seconds: int = None):
        ...
        Args:
            ...
            delay_seconds: Seconds to wait between document uploads (defaults to 30)
        """
        ...
        self.delay_seconds = delay_seconds if delay_seconds is not None else self.UPLOAD_DELAY
    ...
    def _upload_documents_with_delay(self, df: pd.DataFrame) -> int:
        ...
        This is the KEY DIFFERENCE from elasticsearch_upload.py:
        - Uploads documents individually instead of using bulk API
        - Adds 30-second delay between each document upload
        ...
        try:
            ...
            total_docs = len(df)
            logger.info(f"📤 Starting sequential upload of {total_docs} documents with {self.delay_seconds}s delay")
            logger.info(f"⏱️  Estimated total time: {total_docs * self.delay_seconds} seconds ({total_docs * self.delay_seconds / 60:.1f} minutes)")
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    logger.info(f"✅ [{success_count}/{total_docs}] Uploaded: {doc.get('title', 'Untitled')[:60]}...")
                    
                    # Add delay BEFORE next upload (except after last document)
                    if idx < len(df) - 1:  # Not the last document
                        logger.info(f"⏳ Waiting {self.delay_seconds} seconds before next upload...")
                        time.sleep(self.delay_seconds)
                    
                except Exception as e:
                    failed_count += 1
                    logger.error(f"❌ [{success_count + failed_count}/{total_docs}] Failed to upload document: {e}")
                    # Continue with next document even if one fails
                    continue
            
            logger.info(f"✅ Upload completed: {success_count} successful, {failed_count} failed")
    ...
    def _print_summary(self, original_df: pd.DataFrame, new_docs_uploaded: int) -> None:
        ...
        print("\n" + "=" * 70)
        print(f"✅ Elasticsearch upload with delay completed!")
        print(f"⏱️  Upload delay used: {self.delay_seconds} seconds between documents")
    ...
    def upload_to_elasticsearch(self) -> None:
        """
        Main method to upload Excel data to Elasticsearch WITH DELAY (append-only mode).
        """
        try:
            ...
            if uploaded_count > 0:
                logger.info(f"🎉 Successfully uploaded {uploaded_count} new documents with delay!")
            else:
                logger.info("✅ No new documents to upload - all documents already exist!")
            
        except Exception as e:
            logger.error(f"💥 Elasticsearch upload with delay failed: {e}")
            raise
...
def main():
    ...
    try:
        logger.info("=" * 70)
        logger.info("🚀 Elasticsearch Upload Script WITH DELAY")
        logger.info("=" * 70)
        logger.info("📋 Using configuration:")
        logger.info(f"   • Excel file: {ElasticsearchUploaderWithDelay.DEFAULT_EXCEL_PATH}")
        logger.info(f"   • Elasticsearch: {config.ELASTICSEARCH_HOST}")
        logger.info(f"   • Index: {config.ELASTICSEARCH_INDEX}")
        logger.info(f"   • Delay between uploads: {ElasticsearchUploaderWithDelay.UPLOAD_DELAY} seconds")
        logger.info("=" * 70)
        
        # Ask for confirmation
        print("\n⚠️  WARNING: This will upload documents with 30-second delay between each.")
        print(f"    If you have N documents, this will take approximately {ElasticsearchUploaderWithDelay.UPLOAD_DELAY}*N seconds.")
        response = input("\n❓ Continue? (yes/no): ")
